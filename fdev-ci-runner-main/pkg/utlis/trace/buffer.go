...
func (b *Buffer) GetLogFileName() string {
...
func New(job string) (*Buffer, error) {
	//logFile, err := ioutil.TempFile("", "trace")
	//timeStr := time.Now().Format("2006-01-02T15:04:05")
	//filename := fmt.Sprintf("/logs/%s-job-%s", timeStr,job)
	filename := fmt.Sprintf("%s", job)
	abs, err := filepath.Abs(filename)
	if err != nil {
		return nil, err
	}
	if err := os.MkdirAll(filepath.Dir(abs), 0666); err != nil {
		return nil, err
	}
	logFile, err := os.Create(abs)
	if err != nil {
		return nil, err
	}
