...
import java.util.*;
@RefreshScope
@Component(Dict.DemandInfoNotifyImpl)
public class DemandInfoNotifyImpl implements INotifyStrategy{
    ...
    public void doNotify(Map<String, Object> param) {
        try {
            ...
            String mailContent = mailService.getInfoMailContent(hashMap);
            //2. 获取所有使用关联人员
            ...
            String topic = "需求录入提醒";
            mailService.sendEmail(topic, Dict.FDEMAND_INFO_FTL, sendMap, email_receivers.toArray(new String[email_receivers.size()]));
        } catch (Exception e) {
            logger.error("需求录入邮件发送失败{}", e.getStackTrace());
        }
    }
}
