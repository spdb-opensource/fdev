...
import java.util.*;
@RefreshScope
@Component(Dict.DemandRecoverNotifyImpl)
public class DemandRecoverNotifyImpl implements INotifyStrategy {

    private static final Logger logger = LoggerFactory.getLogger(DemandRecoverNotifyImpl.class);
    ...
    public void doNotify(Map<String, Object> param) {
        try {
            ...
            String mailContent = mailService.getRecoverMailContent(hashMap);
            //2. 获取所有使用关联人员
            ...
            String topic = "需求恢复提醒";
            mailService.sendEmail(topic, Dict.FDEMAND_RECOVER_FTL, sendMap, email_receivers.toArray(new String[email_receivers.size()]));
        } catch (Exception e) {
            logger.error("需求恢复邮件发送失败{}", e.getStackTrace());
        }
    }
}
