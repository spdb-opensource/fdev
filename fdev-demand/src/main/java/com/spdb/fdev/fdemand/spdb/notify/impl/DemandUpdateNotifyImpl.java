...
@RefreshScope
@Component(Dict.DemandUpdateNotifyImpl)
public class DemandUpdateNotifyImpl implements INotifyStrategy{
    ...
    public void doNotify(Map<String, Object> param) {
        try {
            ...
            hashMap.put(Dict.OA_CONTACT_NAME, oa_contact_name);
            ...
            String mailContent = mailService.getUpdateMailContent(hashMap);
            //2. 获取所有使用关联人员
            ...
            if (!CommonUtils.isNullOrEmpty(demandBaseInfo.getRelate_part_detail())) {
                demandBaseInfo.getRelate_part_detail().stream().forEach(x ->
                        users.addAll(null != x.getAssess_user() ? x.getAssess_user() : new HashSet<>()));//获取评估人员
            }
            ...
            String topic = "需求更新提醒";
            mailService.sendEmail(topic, Dict.FDEMAND_UPDATE_FTL, sendMap, email_receivers.toArray(new String[email_receivers.size()]));
        } catch (Exception e) {
            logger.error("需求更新邮件发送失败{}", e.getStackTrace());
        }
    }
}
