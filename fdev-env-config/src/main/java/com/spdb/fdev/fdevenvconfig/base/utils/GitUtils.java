...
public class GitUtils {
    ...
    static {
        try {
            Properties local = PropertiesLoaderUtils.loadProperties(new ClassPathResource("application-dev.properties"));
            String userName = local.getProperty("gitlab.name");
            String password = local.getProperty("gitlab.password");
     ...
     * @return
     */
    public static boolean gitPullFromGitlab(String diskDir) throws FdevException {
        boolean pullFlag = true;
        Git git = null;
        try {
            git = Git.open(new File(diskDir + "/.git"));
            git.pull().setRemoteBranchName(Constants.MASTER).setCredentialsProvider(provider).call();
            git.close();
        } catch (Exception e) {
            pullFlag = false;
     ...
     * @param dir    本地存放项目的目录
     */
    public static boolean gitCloneFromGitlab(String gitURI, String dir) throws FdevException {
     ...
     * @throws Exception
     */
    public static void gitPushFromGitlab(String diskDir, String message) throws FdevException {
     ...
     * @throws Exception
     */
    public static boolean createLocalBranchAndRealectRemotBranch(Git git, String localBranchName, String remoteBranchName) {
        boolean flag = false;
        try {
