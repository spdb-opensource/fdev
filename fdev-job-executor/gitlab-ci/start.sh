...
if [[ `echo ${CI_ENVIRONMENT_SLUG} | grep -i master` ]]
then
    java -javaagent:/usr/skywalking/agent/skywalking-agent.jar -jar -Xmx2048m -Xms2048m -Dnetworkaddress.cache.ttl=0 -DSPRING_PROFILE_ACTIVE=$SPRING_PROFILE_ACTIVE -Dspring.cloud.config.uri=xxx/fdev-configserver /ebank/app.jar
else
    java -javaagent:/usr/skywalking/agent/skywalking-agent.jar -jar -Xmx2048m -Xms2048m -Dnetworkaddress.cache.ttl=0 -DSPRING_PROFILE_ACTIVE=$SPRING_PROFILE_ACTIVE -Dspring.cloud.config.uri=xxx/fdev-configserver /ebank/app.jar
fi
