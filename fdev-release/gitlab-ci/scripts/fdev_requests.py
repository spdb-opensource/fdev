...
def query_automation_env(prod_type):
    """
    查询自动化发布环境
    :param prod_type 变更类型（灰度、生产）
    :return:
    """
    data = {"type": prod_type}
...
def download_minio_file(file_path, minio_path, module_name):
    ...
    :return:
    """
    data = {"path": file_path, "full_path": minio_path, "moduleName": module_name}
    response = fdev_request("/frelease/api/release/downloadMinioFile", data)
    return response
...
def update_push_image_status(logging, push_id, status, log):
    """
    根据id修改镜像推送状态以及推送日志
    ...
    :return:
    """
    data = {"id": push_id, "status": status, "push_image_log": log}
    try:
...
def update_by_prod_application(prod, application, status, log):
    """
    ...
    :return:
    """
    data = {"prod_id": prod, "application_id": application, "status": status, "push_image_log": log}
