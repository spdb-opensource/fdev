...
if [[ `echo ${CI_ENVIRONMENT_SLUG} | grep -i master` ]]
then
    java -jar -Xmx2048m -Xms2048m -DSPRING_PROFILE_ACTIVE=$SPRING_PROFILE_ACTIVE -Dspring.cloud.config.uri=http://fdev-configserver-master-svc:8080 /ebank/app.jar
else
    java -jar -Xmx2048m -Xms2048m -DSPRING_PROFILE_ACTIVE=$SPRING_PROFILE_ACTIVE -Dspring.cloud.config.uri=http://fdev-configserver-test-svc:8080 /ebank/app.jar
fi