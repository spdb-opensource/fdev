...
const xhr = isSSR ? null : XMLHttpRequest,
...
function translate({ p, pos, active, horiz, reverse, dir }) {
  ...
  return {
    transform: `translate3d(${active ? 0 : dir * x * -200}%,${y *
      (p - 100)}%,0)`
  };
}
...
export default Vue.extend({
  ...
  computed: {
    classes() {
      return (
        `q-loading-bar q-loading-bar--${this.position}` +
    ...
    style() {
      ...
      const o = translate({
        ...
        reverse:
          this.$q.lang.rtl === true && ['top', 'bottom'].includes(this.position)
            ? !this.reverse
            : this.reverse,
