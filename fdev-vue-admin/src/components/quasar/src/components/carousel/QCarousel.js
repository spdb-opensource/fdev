...
export default Vue.extend({
  ...
  props: {
    ...
    controlType: {
      type: String,
      validator: v =>
        ['regular', 'flat', 'outline', 'push', 'unelevated'].includes(v),
      default: 'flat'
    },
    controlColor: String,
  ...
  computed: {
    ...
    classes() {
      return (
        ...
        (this.navigation === true
          ? ` q-carousel--navigation-${this.navigationPositionComputed}`
          : '')
      );
    },
    ...
    arrowIcons() {
      const ico = [
        this.prevIcon ||
          this.$q.iconSet.carousel[this.vertical === true ? 'up' : 'left'],
        this.nextIcon ||
          this.$q.iconSet.carousel[this.vertical === true ? 'down' : 'right']
      ];
    ...
    navigationPositionComputed() {
      return (
        this.navigationPosition || (this.vertical === true ? 'right' : 'bottom')
      );
    },
  ...
  methods: {
    ...
    __getContent(h) {
      ...
      if (this.navigation === true) {
        const fn =
          this.$scopedSlots['navigation-icon'] !== void 0
        ...
        node.push(
          this.__getNavigationContainer(h, 'buttons', (panel, index) => {
            const { name } = panel.componentOptions.propsData;
