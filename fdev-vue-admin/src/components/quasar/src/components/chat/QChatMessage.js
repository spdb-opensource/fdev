...
export default Vue.extend({
  ...
  computed: {
    textClass() {
      return (
        `q-message-text-content q-message-text-content--${this.op}` +
    ...
    messageClass() {
      return (
        `q-message-text q-message-text--${this.op}` +
  ...
  methods: {
    __getText(h) {
      const domPropText =
          this.textSanitize === true ? 'textContent' : 'innerHTML',
        domPropStamp =
          this.stampSanitize === true ? 'textContent' : 'innerHTML';

      return this.text.map((msg, index) =>
        h(
          'div',
          {
            key: index,
      ...
      this.stamp !== void 0 &&
        content.push(
          h('div', {
            staticClass: 'q-message-stamp',
            domProps: {
              [this.stampSanitize === true ? 'textContent' : 'innerHTML']: this
                .stamp
            }
          })
        );
    ...
    this.name !== void 0 &&
      msg.push(
        h('div', {
          ...
          domProps: {
            [this.nameSanitize === true ? 'textContent' : 'innerHTML']: this
              .name
          }
        })
      );

    this.text !== void 0 && msg.push(this.__getText(h));

    this.$scopedSlots.default !== void 0 && msg.push(this.__getMessage(h));

    container.push(h('div', { class: this.sizeClass }, msg));
    ...
    this.label &&
      child.push(
        h('div', {
          ...
          domProps: {
            [this.labelSanitize === true ? 'textContent' : 'innerHTML']: this
              .label
          }
        })
      );

    child.push(h('div', { class: this.containerClass }, container));
