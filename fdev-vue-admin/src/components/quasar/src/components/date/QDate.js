...
export default Vue.extend({
  ...
  watch: {
    value(v) {
      if (this.lastEmitValue === v) {
        ...
        const { year, month } = this.__getViewModel(
          this.innerMask,
          this.innerLocale
        );
  ...
  computed: {
    classes() {
      ...
      return (
        `q-date q-date--${type} q-date--${type}-${
          this.minimal === true ? 'minimal' : 'standard'
        }` +
    ...
    daysModel() {
      return this.normalizedModel
        ...
        .map(date =>
          this.__decodeString(date, this.innerMask, this.innerLocale)
        )
    ...
    rangeModel() {
      ...
      return this.normalizedModel
        .filter(
          date =>
            Object(date) === date && date.from !== void 0 && date.to !== void 0
        )
    ...
    headerTitle() {
      ...
      if (this.editRange !== void 0) {
        ...
        return (
          this.innerLocale.daysShort[date.getDay()] +
          ', ' +
    ...
    minSelectedModel() {
      const model = this.daysModel
        .concat(this.rangeModel.map(range => range.from))
    ...
    maxSelectedModel() {
      const model = this.daysModel
        .concat(this.rangeModel.map(range => range.to))
    ...
    daysOfWeek() {
      const days = this.innerLocale.daysShort,
    ...
    navBoundaries() {
      ...
      if (this.minNav !== void 0 && this.minNav.year >= this.viewModel.year) {
        ...
        if (
          this.minNav.year === this.viewModel.year &&
          this.minNav.month >= this.viewModel.month
        ) {
      ...
      if (this.maxNav !== void 0 && this.maxNav.year <= this.viewModel.year) {
        ...
        if (
          this.maxNav.year === this.viewModel.year &&
          this.maxNav.month <= this.viewModel.month
        ) {
    ...
    days() {
      ...
      if (
        this.viewModel.year === this.today.year &&
        this.viewModel.month === this.today.month
      ) {
        ...
        if (day.fill === true) {
          ...
          if (day.editRange === true) {
            cls += ` q-date__edit-range${
              day.editRangeFrom === true ? '-from' : ''
            }${day.editRangeTo === true ? '-to' : ''}`;
          }
  ...
  methods: {
    ...
    setEditingRange(from, to) {
      ...
      const final =
        to !== void 0 ? Object.assign({ ...this.viewModel }, to) : init;
    ...
    __getDefaultViewModel() {
      ...
      if (this.defaultYearMonth !== void 0) {
        ...
        const d = this.today !== void 0 ? this.today : this.__getCurrentDate();
    ...
    __getNavigation(h, { label, view, key, dir, goTo, boundaries, cls }) {
      return [
        ...
        h(
          ...
          {
            staticClass:
              'relative-position overflow-hidden flex flex-center' + cls
          },
          [
            h(
              'transition',
          ...
          [
            ...
            h(
              ...
              this.daysOfWeek.map(day =>
                h('div', { staticClass: 'q-date__calendar-item' }, [
                  h('div', [day])
                ])
              )
            ),
                ...
                h(
                  ...
                  [
                    h(
                      ...
                      this.days.map(day =>
                        h('div', { staticClass: day.classes }, [
                          day.in === true
                            ? h(
                                QBtn,
                                {
                                  staticClass:
                                    day.today === true ? 'q-date__today' : null,
                                  props: {
                                    dense: true,
                                    flat: day.flat,
                                    flatColorless: true,
                                  ...
                                  on: cache(this, 'day#' + day.i, {
                                    click: () => {
                                      this.__onDayClick(day.i);
                                    },
                                    mouseover: () => {
                                      this.__onDayMouseover(day.i);
                                    }
                                  })
                                },
                                day.event !== false
    ...
    __getMonthsView(h) {
      ...
      const isDisabled = month => {
        return (
          (this.minNav !== void 0 &&
            this.viewModel.year === this.minNav.year &&
            this.minNav.month > month) ||
          (this.maxNav !== void 0 &&
            this.viewModel.year === this.maxNav.year &&
            this.maxNav.month < month)
        );
      };
      ...
      const content = this.innerLocale.monthsShort.map((month, i) => {
        ...
        return h(
          ...
          [
            h(QBtn, {
              staticClass:
                currentYear === true && this.today.month === i + 1
                  ? 'q-date__today'
                  : null,
              props: {
                flat: active !== true,
      ...
      return h(
        ...
        [
          ...
          h(
            ...
            {
              staticClass:
                'q-date__years-content col self-stretch row items-center'
            },
            years
          ),
    ...
    __toggleDate(date, monthHash) {
      ...
      const fn =
        month !== void 0 && month.includes(date.day) === true
    ...
    __onDayClick(dayIndex) {
      ...
      if (this.editRange === void 0) {
        const dayProps = this.days.find(
          day => day.fill !== true && day.i === dayIndex
        );
        ...
        this.editRange = void 0;
        this.__addToModel(
          initHash === finalHash ? day : { target: day, ...payload }
        );
    ...
    __updateViewModel(year, month) {
      ...
      if (newHash !== this.viewModel.dateHash) {
        this.monthDirection =
          this.viewModel.dateHash < newHash === (this.$q.lang.rtl !== true)
            ? 'left'
            : 'right';
    ...
    __emitValue(val, action, date) {
      const value =
        val !== null && val.length === 1 && this.multiple === false
          ? val[0]
          : val;
    ...
    __emitImmediately(reason) {
      const date =
        this.daysModel[0] !== void 0 && this.daysModel[0].dateHash !== null
          ? this.daysModel[0]
    ...
    __addToModel(date) {
      ...
      if (this.multiple === true) {
        if (date.from !== void 0) {
          ...
          const days = this.daysModel.filter(
            day => day.dateHash < fromHash || day.dateHash > toHash
          );

          const ranges = this.rangeModel.filter(
          ...
          value = days
            .concat(ranges)
            .concat(date)
            .map(entry => this.__encodeEntry(entry));
        } else {
    ...
    __updateValue(mask, locale, reason) {
      ...
      this.$emit(
        'input',
        (this.multiple === true ? model : model[0]) || null,
        reason
      );
    }
  },
  ...
  render(h) {
    ...
    def !== void 0 &&
      content.push(h('div', { staticClass: 'q-date__actions' }, def));
