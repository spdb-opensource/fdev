...
export default Vue.extend({
  ...
  props: {
    ...
    position: {
      ...
      default: 'standard',
      validator: val =>
        val === 'standard' || ['top', 'bottom', 'left', 'right'].includes(val)
    },
  ...
  computed: {
    classes() {
      return (
        `q-dialog__inner--${
          this.maximized === true ? 'maximized' : 'minimized'
        } ` +
        ...
        (this.popupProxy === true ? ' q-popup-proxy' : '')
      );
    },
    ...
    transitionShowComputed() {
      return (
        'q-transition--' +
        (this.transitionShow === void 0
          ? transitions[this.position][0]
          : this.transitionShow)
      );
    },
    ...
    transitionHideComputed() {
      return (
        'q-transition--' +
        (this.transitionHide === void 0
          ? transitions[this.position][1]
          : this.transitionHide)
      );
    },
  ...
  methods: {
    ...
    __show(evt) {
      ...
      this.__refocusTarget =
        this.noRefocus === false && document.activeElement !== null
    ...
    __renderPortal(h) {
      return h(
        'div',
        {
          staticClass: `q-dialog fullscreen no-pointer-events q-dialog--${
            this.useBackdrop === true ? 'modal' : 'seamless'
          }`,
