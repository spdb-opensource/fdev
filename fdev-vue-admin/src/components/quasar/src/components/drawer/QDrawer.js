...
export default Vue.extend({
  ...
  data() {
    const belowBreakpoint =
      ...
      (this.behavior !== 'desktop' &&
        this.layout.totalWidth <= this.breakpoint);

    return {
      belowBreakpoint,
      showing:
        this.showIfAbove === true && belowBreakpoint === false
          ? true
          : this.value === true
    };
  },
  ...
  computed: {
      ...
      return (
        ...
        (this.belowBreakpoint === true
          ...
          : ` q-drawer--${this.isMini === true ? 'mini' : 'standard'}` +
            ...
            (this.overlay === true || this.miniToOverlay === true
              ? ' q-drawer--on-top'
              : '') +
    ...
    stateDirection() {
      return (
        (this.$q.lang.rtl === true ? -1 : 1) *
        (this.rightSide === true ? 1 : -1)
      );
    },
  ...
  methods: {
    ...
    __applyBackdrop(x, retry) {
      if (this.$refs.backdrop !== void 0) {
        // this.$refs.backdrop.style.backgroundColor = this.lastBackdropBg = `rgba(0,0,0,${x *
        //   0.4})`;
        this.$refs.backdrop.style.backgroundColor = 'none';
      } else {
    ...
    __setScrollable(v) {
      const action =
        v === true ? 'remove' : this.layout.container !== true ? 'add' : '';
      ...
      h(
        'div',
        {
          staticClass:
            ...
            (this.layout.container === true ? 'overflow-auto' : 'scroll'),
