...
export default Vue.extend({
  ...
  computed: {
    ...
    buttonProps() {
      const flat = this.toolbarOutline !== true && this.toolbarPush !== true;
    ...
    buttonDef() {
      ...
      return {
        ...
        p: {
          cmd: 'formatBlock',
          param: this.paragraphTag.toUpperCase(),
          icon: i.heading,
          tip: e.paragraph
        },
        code: {
        ...
        group.map(token => {
          ...
          if (obj) {
            return obj.type === 'no-state' ||
              (userDef[token] &&
                (obj.cmd === void 0 ||
                  (this.buttonDef[obj.cmd] &&
                    this.buttonDef[obj.cmd].type === 'no-state')))
              ? obj
    ...
    classes() {
      return (
        `q-editor q-editor--${
          this.isViewingSource === true ? 'source' : 'default'
        }` +
  ...
  methods: {
      ...
      this.$nextTick(() => {
        if (this.$refs.content !== void 0 && this.__offsetBottom !== void 0) {
          this.$refs.content.scrollTop =
            this.$refs.content.scrollHeight - this.__offsetBottom;
        }
      });
      this.$emit('focus', e);
    },
    ...
    runCmd(cmd, param, update = true) {
      ...
      this.caret.apply(cmd, param, () => {
        ...
        this.caret.save();
        if (
          this.$q.platform.is.ie === true ||
          this.$q.platform.is.edge === true
        ) {
    ...
    return h(
      ...
      [
        ...
        h('div', {
          ...
          domProps: isSSR ? { innerHTML: this.value } : undefined,
          on
        })
      ]
    );
