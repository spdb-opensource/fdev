...
function getBtn(h, vm, btn, clickHandler, active = false) {
  const toggled =
      ...
      (btn.type === 'toggle'
        ? btn.toggled
          ? btn.toggled(vm)
          : btn.cmd && vm.caret.is(btn.cmd, btn.param)
        : false),
    child = [],
    events = {
      click(e) {
  ...
  return h(
    QBtn,
    {
      props: {
        ...
        color: toggled
          ? btn.toggleColor || vm.toolbarToggleColor
          : btn.color || vm.toolbarColor,
        textColor:
          toggled && !vm.toolbarPush
            ? null
            : btn.textColor || vm.toolbarTextColor,
        label: btn.label,
        disable: btn.disable
...
function getDropdown(h, vm, btn) {
  ...
  if (onlyIcons) {
    Items = btn.options.map(btn => {
      const active =
        btn.type === void 0 ? vm.caret.is(btn.cmd, btn.param) : false;
    ...
    const activeClass =
      vm.toolbarToggleColor !== void 0 ? `text-${vm.toolbarToggleColor}` : null;
    const inactiveClass =
      vm.toolbarTextColor !== void 0 ? `text-${vm.toolbarTextColor}` : null;
        ...
        [
          noIcons === true
            ...
            : h(
                ...
                [
                  h(QIcon, {
                    props: { name: btn.icon !== null ? btn.icon : void 0 }
                  })
                ]
              ),
...
export function getLinkEditor(h, vm, ie11) {
  if (vm.caret) {
    ...
    return [
      h('div', { staticClass: 'q-mx-xs', class: `text-${color}` }, [
        `${vm.$q.lang.editor.url}: `
      ]),
      h('input', {
        key: 'qedt_btm_input',
