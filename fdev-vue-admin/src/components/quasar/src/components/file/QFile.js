...
export default Vue.extend({
  ...
  props: {
    ...
    value: isSSR === true ? {} : [File, FileList, Array],
  ...
  computed: {
    ...
    selectedString() {
      return this.innerValue.map(file => file.name).join(', ');
    },
    ...
    computedCounter() {
      ...
      return `${this.innerValue.length}${max !== void 0 ? ' / ' + max : ''} (${
        this.totalSize
      })`;
    },
  ...
  methods: {
    ...
    __addFiles(e, fileList) {
      const files = this.__processFiles(
        e,
        fileList,
        this.innerValue,
        this.isAppending
      ...
      files !== void 0 &&
        this.__emitValue(
          this.isAppending === true ? this.innerValue.concat(files) : files
        );
    },
    ...
    __getSelection(h) {
      if (this.$scopedSlots.file !== void 0) {
        return this.innerValue.map((file, index) =>
          this.$scopedSlots.file({ index, file, ref: this })
        );
      }
      ...
      if (this.useChips === true) {
        return this.innerValue.map((file, i) =>
          h(
            QChip,
            {
              key: 'file-' + i,
