...
export default Vue.extend({
  ...
  computed: {
    ...
    revealOnFocus() {
      return (
        this.value === true && this.hidden === true && this.reveal === true
      );
    },

    classes() {
      return (
        (this.fixed === true ? 'fixed' : 'absolute') +
        '-top' +
    ...
    style() {
      const view = this.layout.rows.top,
        css = {'box-shadow': '0 2px 10px 0 rgba(21,101,192,0.20)'};

      if (view[0] === 'l' && this.layout.left.space === true) {
        css[
          this.$q.lang.rtl === true ? 'right' : 'left'
        ] = `${this.layout.left.size}px`;
      }
      if (view[2] === 'r' && this.layout.right.space === true) {
        css[
          this.$q.lang.rtl === true ? 'left' : 'right'
        ] = `${this.layout.right.size}px`;
      }
      return css;
    },
  ...
  render(h) {
    ...
    this.elevated === true &&
      child.push(
        h('div', {
          staticClass:
            'absolute-full overflow-hidden no-pointer-events'
        })
      );
    ...
    return h(
      'header',
      {
        staticClass: 'q-header scroll-thin-auto q-layout__section--marginal',
        class: this.classes,
        style: this.style,
        on: this.onEvents
      },
