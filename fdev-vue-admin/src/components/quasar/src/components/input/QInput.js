...
export default Vue.extend({
  ...
  computed: {
    ...
    fieldClass() {
      return (
        `q-${this.isTextarea === true ? 'textarea' : 'input'}` +
        (this.autogrow === true ? ' q-textarea--autogrow' : '') +
        (this.isTextarea === false && this.large === true ? ' q-input-large' : '')
      );
    },
    ...
    inputAttrs() {
      const attrs = {
        ...
        placeholder: this.qAttrs.placeholder === void 0 ? '请输入' : this.qAttrs.placeholder
      };
