...
export default Vue.extend({
  ...
  computed: {
    anchorOrigin() {
      return parsePosition(
        this.anchor || (this.cover === true ? 'center middle' : 'bottom start'),
        this.$q.lang.rtl
      );
    },
    ...
    menuClass() {
      return (
        (this.noPadding === true ? '' : ' q-px-llg q-py-md') +
        (this.square === true ? ' q-menu--square' : '') +
        (this.isDark === true ? ' q-menu--dark q-dark' : '') +
        (this.isMenu === true
          ? ' no-scroll q-menu--header'
          : ' scroll-thin-y-auto')
      );
    },
  ...
  methods: {
    focus() {
      let node =
        this.__portal !== void 0 && this.__portal.$refs !== void 0
  ...
  beforeDestroy() {
    ...
    if (this.showing === true && this.anchorEl !== void 0) {
      this.anchorEl.dispatchEvent(create('popup-hide', { bubbles: true }));
    }
  }
});
