...
export default Vue.extend({
  ...
  data() {
    ...
    return {
      type:
        this.$q.screen.width < breakpoint || this.$q.screen.height < breakpoint
          ? 'dialog'
          : 'menu'
    };
  },
  ...
  methods: {
    ...
    __updateType(width, height, breakpoint) {
      const type =
        width < breakpoint || height < breakpoint ? 'dialog' : 'menu';
  ...
  render(h) {
    ...
    const data = {
      ref: 'popup',
      props: { ...props, ...this.qAttrs, popupProxy: true },
      on: this.onEvents
    };
    ...
    if (this.type === 'dialog') {
      ...
      data.props.noPadding = this.noPadding;
    }
