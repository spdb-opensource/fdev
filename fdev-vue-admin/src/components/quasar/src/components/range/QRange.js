...
export default Vue.extend({
  ...
  computed: {
    ...
    modelMinRatio() {
      return this.minMaxDiff === 0
        ? 0
        : (this.model.min - this.min) / this.minMaxDiff;
    },

    modelMaxRatio() {
      return this.minMaxDiff === 0
        ? 0
        : (this.model.max - this.min) / this.minMaxDiff;
    },
    ...
    minPinStyle() {
      const percent =
        this.reverse === true ? -this.ratioMin : this.ratioMin - 1;
      return this.__getPinStyle(percent, this.ratioMin);
    },

    maxPinStyle() {
      const percent =
        this.reverse === true ? -this.ratioMax : this.ratioMax - 1;
  ...
  methods: {
    __updateValue(change) {
      if (
        this.model.min !== this.value.min ||
        this.model.max !== this.value.max
      ) {
    ...
    __keydown(evt) {
      ...
      const step =
          ([34, 33].includes(evt.keyCode) ? 10 : 1) * this.computedStep,
    ...
    return h(
      'div',
      {
        ...
        attrs: {
          ...
          tabindex:
            this.dragOnlyRange === true && this.$q.platform.is.mobile !== true
