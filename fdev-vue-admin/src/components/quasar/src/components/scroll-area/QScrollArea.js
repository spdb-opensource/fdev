...
export default Vue.extend({
  ...
  computed: {
    classes() {
      return (
        'q-scrollarea' + (this.isDark === true ? ' q-scrollarea--dark' : '')
      );
    },
    ...
    scrollPercentage() {
      const p = between(
        this.scrollPosition / (this.scrollSize - this.containerSize),
        0,
        1
      );
    ...
    thumbClass() {
      return (
        `q-scrollarea__thumb--${this.dirProps.classSuffix}` +
    ...
    barClass() {
      return (
        `q-scrollarea__bar--${this.dirProps.classSuffix}` +
  ...
  methods: {
    ...
    __panThumb(e) {
      ...
      const multiplier =
        (this.scrollSize - this.containerSize) /
        (this.containerSize - this.thumbSize);
      const distance = e.distance[this.dirProps.dist];
      const pos =
        this.refPos +
        (e.direction === this.dirProps.dir ? 1 : -1) * distance * multiplier;
    ...
    __mouseDown(evt) {
      if (this.thumbHidden !== true) {
        const pos =
          evt[`offset${this.horizontal === true ? 'X' : 'Y'}`] -
          this.thumbSize / 2;
