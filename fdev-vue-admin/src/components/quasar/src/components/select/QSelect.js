...
export default Vue.extend({
    ...
    innerValue: {
      handler(val) {
        ...
        if (
          ...
          ((this.dialog !== true && this.menu !== true) ||
            this.hasValue !== true)
        ) {
  ...
  computed: {
    useInputPlaceholder() {
      return this.displayValue !== void 0 || this.selectedString
        ? ''
        : this.placeholder;
    },
    ...
    fieldClass() {
      return (
        `q-select q-field--auto-height q-select--with${
          this.useInput !== true ? 'out' : ''
        }-input` +
        `${this.flat === true ? ' q-select--flat' : ''}` +
        ` q-select--with${
          this.flat !== true &&
          (this.multiple === true || this.useChips === true)
            ? ''
            : 'out'
        }-chips` +
        ` q-select--${this.multiple === true ? 'multiple' : 'single'}` +
        `${this.hasValue ? ' q-select--hasValue' : ''}`
      );
    },
    ...
    menuContentClass() {
      return (
        (this.virtualScrollHorizontal === true
    ...
    innerValue() {
      const mapNull = this.mapOptions === true && this.multiple !== true,
        val =
          this.value !== void 0 && (this.value !== null || mapNull === true)
            ? this.multiple === true && Array.isArray(this.value)
              ? this.value
              : [this.value]
            : [];
      ...
      if (this.mapOptions === true && Array.isArray(this.options) === true) {
        const cache =
          this.mapOptions === true && this.innerValueCache !== void 0
    ...
    displayAsText() {
      return (
        this.displayValueSanitize === true ||
  ...
  methods: {
    ...
    add(opt, unique) {
      ...
      if (this.multiple !== true) {
        this.fillInput === true &&
          this.updateInputValue(this.getOptionLabel(opt), true, true);
    ...
    toggleOption(opt, keepOpen) {
      if (
        this.editable !== true ||
        opt === void 0 ||
        this.isOptionDisabled(opt) === true
      ) {
        return;
      }
      ...
      if (this.multiple !== true) {
        ...
        if (
          isDeepEqual(this.getOptionValue(this.innerValue[0]), optValue) !==
          true
        ) {
    ...
    __onTargetKeydown(e) {
      ...
      const newValueModeValid =
        ...
        (this.newValueMode !== void 0 ||
          this.qListeners['new-value'] !== void 0);
      ...
      if (
        ...
        this.optionIndex = Math.max(
          ...
          Math.min(
            ...
            this.optionIndex +
              (e.keyCode === 33 ? -1 : 1) *
                this.virtualScrollSliceSizeComputed.view
          )
        );
      ...
      if (
        ...
        const char = e.key.toLocaleLowerCase(),
          keyRepeat =
            this.searchBuffer.length === 1 && this.searchBuffer[0] === char;
    ...
    __getSelection(h) {
      ...
      if (this.$scopedSlots['selected-item'] !== void 0) {
        return this.selectedScope
          .map(scope => this.$scopedSlots['selected-item'](scope))
          .slice();
      }
      ...
      if (
        (this.flat !== true && this.multiple === true) ||
        this.useChips === true
      ) {
        return this.selectedScope.map((scope, i) =>
          h(
            QChip,
            {
              key: 'option-' + i,
              props: {
                ...
                removable:
                  this.editable === true &&
                  this.isOptionDisabled(scope.opt) !== true,
                dense: false,
                square: true,
                textColor: 'blue-9',
                tabindex: this.computedTabindex,
            ...
            [
              h('span', {
                ...
                domProps: {
                  [scope.sanitize === true
                    ? 'textContent'
                    : 'innerHTML']: this.getOptionLabel(scope.opt)
                },
                attrs: {
                  title: this.getOptionLabel(scope.opt)
                }
              })
            ]
          )
      ...
      if (this.tableBottomSelect === true && this.displayValue !== '全部') {
        return [
          h('span', {
            staticClass: 'row-page text-no-wrap text-grey-3',
            domProps: {
              [this.displayAsText ? 'textContent' : 'innerHTML']: '行/页'
            }
          }),
          h('span', {
            attrs: {
              title:
                this.displayValue !== void 0
                  ? this.displayValue
                  : this.useInput === true || this.selectedString
                  ? this.selectedString
                  : ''
            },
            staticClass:
              (this.multiple ? '' : 'text-ellipsis text-no-wrap') +
              (this.displayValue === void 0 && !this.selectedString
                ? ' text-black-0'
                : ''),
            style: { width: '25px' },
            domProps: {
              [this.displayAsText ? 'textContent' : 'innerHTML']:
                this.displayValue !== void 0
                  ? this.displayValue
                  : this.useInput === true || this.selectedString
                  ? this.selectedString
          ...
          h('span', {
            ...
            staticClass:
              ...
              (this.displayValue === void 0 && !this.selectedString
                ? ' text-grey-3'
                : '') +
              (this.tableBottomSelect === true && this.displayValue === '全部'
                ? ' row justify-center'
                : ''),
            style: {
              width:
    ...
    __getControl(h, fromDialog) {
      ...
      const isTarget =
        fromDialog === true || this.dialog !== true || this.hasDialog !== true;

      if (
        (this.useInput === true || this.multiple === true) &&
        this.flat !== true
      ) {
      ...
      // there can be only one (when dialog is opened the control in dialog should be target)
        ...
        if (
          typeof this.autocomplete === 'string' &&
      ...
      if (
        this.nameProp !== void 0 &&
        this.disable !== true &&
        this.innerOptionsValue.length > 0
      ) {
        const opts = this.innerOptionsValue.map(value =>
          h('option', {
      ...
      return h(
        ...
        {
          staticClass:
            this.useChips || this.multiple
              ? 'q-field__native row items-center'
              : 'q-field__native row items-center no-wrap',
          attrs: this.qAttrs
        },
        child
      );
    ...
    __getOptions(h) {
                ...
                [
                  h(QItemSection, [
                    h(QItemLabel, {
                      ...
                      attrs: { title: this.getOptionLabel(scope.opt) },
      ...
      let options = this.__padVirtualScroll(
        h,
        'div',
        this.optionScope.map((scope, i) => {
    ...
    __getInput(h, fromDialog, isTarget) {
      const options = {
        ...
        attrs: {
          ...
          type: 'search',
          ...this.qAttrs,
          placeholder: this.useInputPlaceholder,
          id: this.targetUid,
    ...
    filter(val, keepClosed) {
      if (
        this.qListeners.filter === void 0 ||
        (keepClosed !== true && this.focused !== true)
      ) {
        return;
      }
      ...
      return h(
        QMenu,
        {
          ...
          props: {
            ...
            contentClass:
              this.menuContentClass +
              ' q-select--menu' +
              `${this.multiple === true ? ' q-select-multiple--menu' : ''}` +
              `${this.flat !== true ? ' q-select-noflat--menu' : ''}`,
    ...
    __onDialogBeforeHide() {
      this.$refs.dialog.__refocusTarget = this.$el.querySelector(
    ...
    __updateMenu(show) {
      ...
      if (show === true) {
        if (this.innerValue.length > 0) {
          ...
          optionIndex = this.options.findIndex(v =>
            isDeepEqual(this.getOptionValue(v), val)
          );
        }
    ...
    __onPreRender() {
      this.hasDialog =
        this.$q.platform.is.mobile !== true && this.behavior !== 'dialog'
          ? false
          : this.behavior !== 'menu' &&
            (this.useInput === true
              ? this.$scopedSlots['no-option'] !== void 0 ||
