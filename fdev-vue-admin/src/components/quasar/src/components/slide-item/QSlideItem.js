...
export default Vue.extend({
  ...
  methods: {
    ...
    __emitSlide(side, ratio, isReset) {
      this.qListeners.slide !== void 0 &&
        this.$emit('slide', { side, ratio, isReset });
    },
    ...
    __pan(evt) {
      ...
      if (evt.isFirst) {
        ...
        this.__axis =
          evt.direction === 'up' || evt.direction === 'down' ? 'Y' : 'X';
      } else if (evt.isFinal) {
        node.classList.remove('no-transition');

        if (this.__scale === 1) {
          node.style.transform = `translate${this.__axis}(${this.__dir *
            100}%)`;
      ...
      if (
        (this.$scopedSlots.left === void 0 &&
          evt.direction === this.langDir.right) ||
        (this.$scopedSlots.right === void 0 &&
          evt.direction === this.langDir.left) ||
      ...
      if (this.__dir !== dir) {
        ['left', 'right', 'top', 'bottom'].forEach(d => {
          if (this.$refs[d] !== void 0) {
            this.$refs[d].style.visibility =
              showing === d ? 'visible' : 'hidden';
          }
        });
      ...
      this.__scale = Math.max(
        0,
        Math.min(1, (dist - 40) / this.__size[showing])
      );

      node.style.transform = `translate${this.__axis}(${(dist * dir) /
        Math.abs(dir)}px)`;
      this.$refs[
        `${showing}Content`
      ].style.transform = `scale(${this.__scale})`;
        ...
        content.push(
          h(
            'div',
            {
              ref: dir,
              class:
                `q-slide-item__${dir} absolute-full row no-wrap items-${slot[1]} justify-${slot[2]}` +
                (this[dir + 'Color'] !== void 0
                  ? ` bg-${this[dir + 'Color']}`
                  : '')
            },
