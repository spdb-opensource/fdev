...
export const SliderMixin = {
  ...
  computed: {
    ...
    classes() {
      return (
        `q-slider q-slider${this.axis} q-slider--${
          this.active === true ? '' : 'in'
        }active` +
        ...
        (this.dense === true
          ? ' q-slider--dense q-slider--dense' + this.axis
          : '')
      );
    },
    ...
    editable() {
      return (
        this.disable !== true && this.readonly !== true && this.min < this.max
      );
    },
    ...
    markerStyle() {
      if (this.minMaxDiff !== 0) {
        ...
        return {
          backgroundSize:
            this.vertical === true ? '2px ' + size + '%' : size + '% 2px'
        };
      }
    },
      ...
      return {
        ...
        pinTextContainer: {
          ...
          transform: `translateX(${Math.ceil(
            (this.$q.lang.rtl === true ? -1 : 1) * 20 * percent
          )}px)`
        }
      };
    },
