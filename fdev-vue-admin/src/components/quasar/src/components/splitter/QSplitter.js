...
export default Vue.extend({
  ...
  computed: {
    classes() {
      return (
        (this.horizontal === true ? 'column' : 'row') +
  ...
  methods: {
    __pan(evt) {
      if (evt.isFirst === true) {
        ...
        this.__value = Math.min(
          this.__maxValue,
          this.computedLimits[1],
          Math.max(this.computedLimits[0], this.value)
        );
        this.__multiplier =
          (this.reverse !== true ? 1 : -1) *
      ...
      this.__normalized = Math.min(
        this.__maxValue,
        this.computedLimits[1],
        Math.max(this.computedLimits[0], val)
      );

      this.$refs[this.side].style[this.prop] = this.__getCSSValue(
        this.__normalized
      );
