...
export default Vue.extend({
  ...
  computed: {
    ...
    headerNav() {
      const opt = this.step.headerNav,
        nav = opt === true || opt === '' || opt === void 0;

      return this.isDisable === false && this.stepper.headerNav && nav;
    },
    ...
    hasPrefix() {
      return (
        ...
        ((this.step.prefix &&
          ...
          (this.isActive === true &&
            this.isDone === false &&
            this.step.activePrefix !== void 0) ||
          (this.isDone === true && this.step.donePrefix !== void 0))
      );
    },
    ...
    icon() {
      if (this.isActive === true) {
        return (
          this.step.activeIcon ||
          this.stepper.activeIcon ||
          this.$q.iconSet.stepper.active
        );
      }
      if (this.isError === true) {
        return (
          this.step.errorIcon ||
          this.stepper.errorIcon ||
          this.$q.iconSet.stepper.error
        );
      }
      if (this.isDisable === false && this.isDone === true) {
        return (
          this.step.doneIcon ||
          this.stepper.doneIcon ||
          this.$q.iconSet.stepper.done
        );
      }
    ...
    color() {
      ...
      if (this.isActive === true) {
        const color =
          this.step.activeColor || this.stepper.activeColor || this.step.color;
      ...
      if (this.isDisable === false && this.isDone === true) {
        return (
          this.step.doneColor ||
          this.stepper.doneColor ||
          this.step.color ||
          this.stepper.inactiveColor
        );
      }
  ...
  render(h) {
    ...
    if (this.hasPrefix === true) {
      if (this.isError === false && this.isDisable === false) {
        prefix = this.isActive ? this.step.activePrefix : this.isDone ? this.step.donePrefix : this.step.prefix;
      }
    }
      ...
      h(
        'div',
        {
          staticClass:
            ...
            `${this.hasPrefix === true ? ' q-stepper__dot--hasprefix' : ''}`
        },
        [
