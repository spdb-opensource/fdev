...
export default Vue.extend({
      ...
      innerPagination: Object.assign(
        {
          ...
          page: 1,
          rowsPerPage:
            this.rowsPerPageOptions.length > 0 ? this.rowsPerPageOptions[0] : 5
        },
        this.pagination
      )
    };
  ...
  watch: {
    needsReset() {
      this.hasVirtScroll === true &&
        this.$refs.virtScroll !== void 0 &&
        this.$refs.virtScroll.reset();
    }
  },
  ...
  computed: {
    ...
    __containerClass() {
      return (
        `q-table__container q-table--${this.separator}-separator column no-wrap` +
  ...
  render(h) {
    ...
    child.push(this.__getBody(h), this.__getBottomDiv(h));
  ...
  methods: {
    ...
    __getBody(h) {
      ...
      if (this.hasVirtScroll === true) {
        ...
        if (bottomRow !== void 0) {
          virtSlots.after = () =>
            h('tbody', bottomRow({ cols: this.computedCols }));
        }
      ...
      return getTableMiddle(
        h,
        {
          staticClass: 'scroll-normal',
          class: this.tableClass,
          style: this.tableStyle
        },
        [header, this.__getTBody(h)]
      );
    },
