...
import QBanner from '../banner/QBanner.js';
...
export default {
  methods: {
    __getTBodyTR(h, row, bodySlot, pageIndex) {
      ...
      const bodyCell = this.$scopedSlots['body-cell'],
        child = this.computedCols.map(col => {
          const bodyCellCol = this.$scopedSlots[`body-cell-${col.name}`],
            slot = bodyCellCol !== void 0 ? bodyCellCol : bodyCell,
            cellValue = [this.getCellValue(col, row)];
          const c = [cellValue];
          col.copy === true &&
            c.push(
              h(QPopupProxy, { props: { 'context-menu': true } }, [
                h(QBanner, { style: { 'max-width': '300px' } }, cellValue)
              ])
            );
    ...
    __getTBody(h) {
      const { body } = this.$scopedSlots,
        topRow = this.$scopedSlots['top-row'],
        bottomRow = this.$scopedSlots['bottom-row'];

      let child = this.computedRows.map((row, pageIndex) =>
        this.__getTBodyTR(h, row, body, pageIndex)
      );
    ...
    getCellValue(col, row) {
      ...
      let tdVal = col.format !== void 0 ? col.format(val, row) : val;
      return tdVal || '-';
    }
  }
};
