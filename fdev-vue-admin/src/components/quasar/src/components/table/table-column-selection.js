...
export default {
  ...
  data() {
    return {
      visibleCols: this.visibleColumns || this.columns.map(x => x.name)
    };
  },
  watch: {
    visibleColumns(val) {
      this.visibleCols = val || this.columns.map(x => x.name);
    }
  },
  computed: {
    colList() {
      ...
      const cols =
        this.visibleCols.length > 0
          ? this.colList.filter(
              col =>
                col.required === true ||
                this.visibleCols.includes(col.name) === true
            )
          : this.colList;
      return cols.map(col => {
        ...
        return {
          ...
          __thClass:
            ...
            (col.name === sortBy
              ...
              : ''),
          __tdClass: `text-${align}${
            col.classes !== void 0 ? ' ' + col.classes : ''
          }`
        };
      });
    },
