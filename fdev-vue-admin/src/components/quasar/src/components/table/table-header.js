...
export default {
  ...
  methods: {
    ...
    __getTHeadTR(h) {
      const { header } = this.$scopedSlots,
        headerCell = this.$scopedSlots['header-cell'];

      if (header !== void 0) {
        return header(this.__getHeaderScope({ header: true })).slice();
      }

      const child = this.computedCols.map(col => {
        const headerCellCol = this.$scopedSlots[`header-cell-${col.name}`],
      ...
      if (this.singleSelection === true && this.grid !== true) {
        child.unshift(
          h('th', { staticClass: 'q-table--col-auto-width' }, [' '])
        );
