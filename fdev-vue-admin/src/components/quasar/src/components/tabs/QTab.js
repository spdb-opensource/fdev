...
export default Vue.extend({
  ...
  computed: {
    ...
    classes() {
      return {
        ...
        [`text-${this.tabs.activeColor}`]:
          this.isActive && this.tabs.activeColor,
        [`bg-${this.tabs.activeBgColor}`]:
          this.isActive && this.tabs.activeBgColor,
    ...
    computedTabIndex() {
      return this.disable === true || this.isActive === true
        ? -1
        : this.tabindex || 0;
    },
  ...
  methods: {
    ...
    __getContent(h) {
      ...
      this.label !== void 0 &&
        content.push(
          h(
            'div',
            {
              staticClass: 'q-tab__label row justify-center'
            },
            [this.label]
          )
        );
      ...
      const node = [
        ...
        h(
          ...
          {
            staticClass:
              'q-tab__content self-stretch flex-start relative-position q-anchor--skip non-selectable',
    ...
    __renderTab(h, tag, props) {
      const data = {
        staticClass: 'q-tab relative-position self-stretch flex text-start',
        class: this.classes,
        attrs: this.attrs,
        directives:
          this.ripple !== false && this.disable === true
            ? null
