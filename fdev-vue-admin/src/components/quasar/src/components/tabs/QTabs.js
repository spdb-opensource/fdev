...
function getIndicatorClass(color, top, vertical) {
  ...
  const pos = vertical === true ? ['left', 'bottom'] : ['top', 'bottom'];

  return `absolute-${top === true ? pos[0] : pos[1]}${
    color ? ` text-${color}` : ''
  }`;
}
...
export default Vue.extend({
  ...
  watch: {
    ...
    vertical(v) {
      this.tabs.indicatorClass = getIndicatorClass(
        this.indicatorColor,
        this.switchIndicator,
        v
      );
  ...
  computed: {
    ...
    classes() {
      return (
        ...
        ` q-tabs__arrows--${
          this.arrowsEnabled === true && this.outsideArrows === true
            ? 'outside'
            : 'inside'
        }` +
  ...
  methods: {
    ...
    __animate(oldName, newName) {
      ...
      if (oldTab && newTab) {
        ...
        newEl.style.transform = `translate3d(${oldPos.left -
          newPos.left}px,0,0) scale3d(${
          newPos.width ? oldPos.width / newPos.width : 1
        },1,1)`;
        // this.vertical === true
        //   ? `translate3d(0,${oldPos.top - newPos.top}px,0) scale3d(1,${
        //       newPos.height ? oldPos.height / newPos.height : 1
        //     },1)`
        //   : `translate3d(${oldPos.left - newPos.left}px,0,0) scale3d(${
        //       newPos.width ? oldPos.width / newPos.width : 1
        //     },1,1)`;
      ...
      if (newTab && this.scrollable === true) {
        ...
        let offset =
          this.vertical === true ? newPos.top - top : newPos.left - left;

        if (offset < 0) {
          this.$refs.content[
            this.vertical === true ? 'scrollTop' : 'scrollLeft'
          ] += Math.floor(offset);
          this.__updateArrows();
          return;
        }
  ...
  created() {
    this.buffer = [];
    this.__updateArrows =
      this.arrowsEnabled === true ? this.__updateArrowsFn : noop;
  },
        ...
        'div',
        {
          ...
          on:
            this.arrowsEnabled === true
              ? cache(this, 'scroll', { scroll: this.__updateArrowsFn })
              : void 0
        },
        slot(this, 'default')
      )
        ...
        h(QIcon, {
          ...
          props: {
            name:
              this.leftIcon ||
              (this.vertical === true
                ? this.$q.iconSet.tabs.up
                : this.$q.iconSet.tabs.left)
          },
          on: cache(this, 'onL', {
        ...
        h(QIcon, {
          ...
          props: {
            name:
              this.rightIcon ||
              (this.vertical === true
                ? this.$q.iconSet.tabs.down
                : this.$q.iconSet.tabs.right)
          },
          on: cache(this, 'onR', {
