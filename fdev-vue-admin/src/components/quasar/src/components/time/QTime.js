...
export default Vue.extend({
  ...
  computed: {
    classes() {
      return (
        `q-time q-time--${this.landscape === true ? 'landscape' : 'portrait'}` +
  ...
  methods: {
    ...
    __getSnapGrid(inSel, count) {
      ...
      const snappingGrid = Array.apply(null, { length: count }).map(
        (_, index) => inSel(index)
      );
      ...
      for (let i = 0; i < count; i++) {
        if (snappingGrid[i] === true) {
          if (consecutiveGaps > 1) {
            ...
            for (
              let j = 0, h = previousValStart;
              j < sideCount;
              j++, h = (previousValStart + j + count) % count
            ) {
            ...
            for (
              let j = 0, h = currentValStart;
              j < sideCount;
              j++, h = (currentValStart + j + count) % count
            ) {
    ...
    __updateClock(evt, clockRect, cacheVal) {
      ...
      if (this.view === 'Hour') {
        ...
        if (this.view === 'Minute' && this.minuteSnappingGrid !== void 0) {
          ...
          this.view === 'Second' &&
          this.secondSnappingGrid !== void 0
        ) {
    ...
    __onKeyupHour(e) {
      if (e.keyCode === 13) {
        ...
        const wrap = this.computedFormat24h === true ? 24 : 12,
          offset =
            this.computedFormat24h !== true && this.isAM === false ? 12 : 0;
                            ...
                            [
                              ...
                              this.positions.map(pos =>
                                h(
                                  'div',
                                  {
                                    staticClass: `q-time__clock-position row flex-center q-time__clock-pos-${
                                      pos.index
                                    }`,
                                    class:
                                      pos.val === current
    ...
    __verifyAndUpdate() {
      if (
        this.hourInSelection !== void 0 &&
        this.hourInSelection(this.innerModel.hour) !== true
      ) {
      ...
      if (
        this.minuteInSelection !== void 0 &&
        this.minuteInSelection(this.innerModel.minute) !== true
      ) {
        this.innerModel.minute = null;
        this.innerModel.second = null;
        this.view = 'Minute';
      ...
      if (
        ...
        this.secondInSelection !== void 0 &&
        this.secondInSelection(this.innerModel.second) !== true
      ) {
        this.innerModel.second = null;
        this.view = 'Second';
        return;
      ...
      if (
        this.innerModel.hour === null ||
        this.innerModel.minute === null ||
        (this.withSeconds === true && this.innerModel.second === null)
      ) {
        return;
      }
  ...
  render(h) {
    const child = [this.__getClock(h)];

    const def = slot(this, 'default');
    def !== void 0 &&
      child.push(h('div', { staticClass: 'q-time__actions' }, def));
