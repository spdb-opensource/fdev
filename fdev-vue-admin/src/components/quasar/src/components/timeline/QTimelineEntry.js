  ...
  computed: {
    ...
    classes() {
      return (
        ...
        (this.icon !== void 0 || this.avatar !== void 0
    ...
    reverse() {
      return (
        this.__timeline.layout === 'comfortable' &&
        this.__timeline.side === 'left'
      );
    }
  },
  ...
  render(h) {
    ...
    if (this.heading === true) {
      const content = [
        h('div'),
        h('div'),
        h(this.tag, { staticClass: 'q-timeline__heading_title' }, child)
      ];
    ...
    const content = [
      titleSlot !== void 0
        ? h('div', {}, titleSlot())
        : h('div', { staticClass: 'text-caption q-mb-xs text-grey-3' }, [
            h('span', slot(this, 'title', [this.title]))
          ]),
