...
export default Vue.extend({
  ...
  computed: {
    ...
    meta() {
      ...
      const travel = (node, parent) => {
        const tickStrategy =
          node.tickStrategy ||
          (parent ? parent.tickStrategy : this.tickStrategy);
        const key = node[this.nodeKey],
          isParent =
            node[this.childrenKey] && node[this.childrenKey].length > 0,
        ...
        const m = {
          ...
          link:
            node.disabled !== true &&
            (selectable === true ||
              (expandable === true && (isParent === true || lazy === true))),
          children: [],
          matchesFilter: this.filter
            ? this.filterMethod(node, this.filter)
            : true,

          selected: key === this.selected && selectable === true,
          selectable,
          expanded:
            isParent === true ? this.innerExpanded.includes(key) : false,
          expandable,
          noTick:
            node.noTick === true ||
            (strictTicking !== true && lazy && lazy !== 'loaded'),
          tickable,
          tickStrategy,
          hasTicking,
          strictTicking,
          ...
          if (m.matchesFilter === true) {
            ...
            if (leafTicking) {
              ...
              m.indeterminate = m.children.some(
                node => node.indeterminate === true
              );
              m.tickable =
                m.tickable === true && m.children.some(node => node.tickable);

              if (m.indeterminate !== true) {
                const sel = m.children.reduce(
                  (acc, meta) => (meta.ticked === true ? acc + 1 : acc),
                  0
                );
              ...
              if (m.indeterminate === true) {
                m.indeterminateNextState = m.children.every(
                  meta => meta.tickable !== true || meta.ticked !== true
                );
              }
            }
          }
  ...
  methods: {
    ...
    expandAll() {
      const expanded = this.innerExpanded,
        travel = node => {
    ...
    __getNode(h, node) {
      ...
      const isParent =
        children.length > 0 || (meta.lazy && meta.lazy !== 'loaded');
      ...
      if (body !== void 0) {
        body = h(
          ...
          [h('div', { class: this.textColorClass }, [body(slotScope)])]
        );
      }
    ...
    __onClick(node, meta, e, keyboard) {
      ...
      if (this.hasSelection) {
        if (meta.selectable) {
          this.$emit(
            'update:selected',
            meta.key !== this.selected ? meta.key : null
          );
        }
      } else {
