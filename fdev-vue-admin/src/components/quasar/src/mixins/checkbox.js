...
import { checkboxOptionSizeMixin } from './option-size.js';
...
export default {
  mixins: [DarkMixin, checkboxOptionSizeMixin, FormMixin, RefocusTargetMixin],
  ...
  computed: {
    ...
    classes() {
      return (
        `q-${this.type} cursor-pointer no-outline row inline no-wrap items-center` +
    ...
    innerClass() {
      // let state;
      // if (this.type === 'toggle') {
      //   state = this.isTrue === true ? 'truthy' : (this.isFalse === true ? 'falsy' : 'indet');
      // } else {
      //   state =
      //   this.disable === true
      //     ? this.isTrue === true
      ...
      const large = this.type === 'toggle' && this.size === 'lg' ? ' large' : '';

      return `q-${this.type}__inner--${state}${large}${color}`;
    },
    ...
    const child = [
      h(
        'div',
        {
          ...
          style: { ...this.sizeStyle, margin: '1px 8px 0 0' }
        },
        inner
      )
    ];
