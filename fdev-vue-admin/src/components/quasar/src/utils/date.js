...
export function __splitDate(str, mask, dateLocale, calendar, defaultModel) {
  ...
  const langOpts = dateLocale !== void 0 ? dateLocale : lang.props.date,
    { months } = langOpts,
    { monthsShort } = langOpts;
  ...
  if (map.X !== void 0 || map.x !== void 0) {
    ...
    if (map.Z !== void 0 || map.ZZ !== void 0) {
      ...
      date.timezoneOffset =
        (tzString[0] === '+' ? -1 : 1) *
        (60 * tzString.slice(1, 3) + 1 * tzString.slice(3, 5));
    }
  }

  date.dateHash = date.year + '/' + pad(date.month) + '/' + pad(date.day);
  date.timeHash =
    pad(date.hour) + ':' + pad(date.minute) + ':' + pad(date.second) + tzString;
...
function getChange(date, mod, add) {
  ...
  Object.keys(mod).forEach(key => {
    ...
    const op =
      key === 'year' ? 'FullYear' : capitalize(key === 'days' ? 'date' : key);
...
export function adjustDate(date, mod, utc) {
  ...
  Object.keys(mod).forEach(key => {
    ...
    const op =
      key === 'year' ? 'FullYear' : key.charAt(0).toUpperCase() + key.slice(1);
    t[`${prefix}${op}`](mod[key]);
  });
...
export function formatDate(
  ...
  const locale = dateLocale !== void 0 ? dateLocale : lang.props.date;
