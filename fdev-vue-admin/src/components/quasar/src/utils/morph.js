...
export default function morph(_options) {
  ...
  const calculateFinalState = () => {
    ...
    // we clean other morphs running on this element
    elFrom !== elTo &&
      typeof elTo.qMorphCancel === 'function' &&
      elTo.qMorphCancel();
    ...
    const animate = () => {
      ...
      if (elFromTween !== void 0) {
        ...
        elFromTween.style.left = `${elFromPosition.left -
          documentScroll.scrollLeft}px`;
        elFromTween.style.right = 'unset';
        elFromTween.style.top = `${elFromPosition.top -
          documentScroll.scrollTop}px`;
      ...
      const commonCleanup = aborted => {
        ...
        elToClone.parentNode === elToParent &&
      ...
      if (options.useCSS !== true && typeof elTo.animate === 'function') {
        ...
        const resizeToTween =
          options.resize === true
            ...
            : {
                transform: `translate(${-1 * deltaX}px, ${-1 *
                  deltaY}px) scale(${1 / scaleX}, ${1 / scaleY})`
              };
        const tweenFrom =
          elFromTween !== void 0
          ...
          options.hideFromClone === true || elSharedSize === true
            ...
            : elFromClone.animate(
                [
                  {
                    margin: `${
                      elFromParentHeightDiff < 0
                        ? elFromParentHeightDiff / 2
                        : 0
                    }px ${
                      elFromParentWidthDiff < 0 ? elFromParentWidthDiff / 2 : 0
                    }px`,
          ...
          options.keepToClone === true
            ...
            : elToClone.animate(
                [
                  elSharedSize === true
                    ? {
                        margin: `${
                          ...
                          elFromParentWidthDiff < 0
                            ? elFromParentWidthDiff / 2
                            : 0
                        }px`,
                        width: `${elFromCloneWidth + elFromPosition.marginH}px`,
                        height: `${elFromCloneHeight +
                          elFromPosition.marginV}px`
                      }
                    : {
                        margin: 0,
                        width: 0,
                  ...
                  {
                    margin: `${
                      elToParentHeightDiff < 0 ? elToParentHeightDiff / 2 : 0
                    }px ${
                      elToParentWidthDiff < 0 ? elToParentWidthDiff / 2 : 0
                    }px`,
        ...
        const keyframesFrom =
          options.hideFromClone === true || elSharedSize === true
            ...
            @keyframes ${qAnimId}-from {
              0% {
                margin: ${
                  elFromParentHeightDiff < 0 ? elFromParentHeightDiff / 2 : 0
                }px ${
                elFromParentWidthDiff < 0 ? elFromParentWidthDiff / 2 : 0
              }px;
                width: ${elFromCloneWidth + elFromPosition.marginH}px;
        ...
        const keyframeToStart =
          elSharedSize === true
            ...
            margin: ${
              elFromParentHeightDiff < 0 ? elFromParentHeightDiff / 2 : 0
            }px ${elFromParentWidthDiff < 0 ? elFromParentWidthDiff / 2 : 0}px;
            width: ${elFromCloneWidth + elFromPosition.marginH}px;
            height: ${elFromCloneHeight + elFromPosition.marginV}px;
          `
            : `
          ...
          options.keepToClone === true
            ...
            @keyframes ${qAnimId}-to {
              ...
              100% {
                margin: ${
                  elToParentHeightDiff < 0 ? elToParentHeightDiff / 2 : 0
                }px ${elToParentWidthDiff < 0 ? elToParentWidthDiff / 2 : 0}px;
                width: ${elToCloneWidth + elToPosition.marginH}px;
                height: ${elToCloneHeight + elToPosition.marginV}px;
              }
            }
