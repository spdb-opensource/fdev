      ...
      >
        <template v-slot:top-right>
          <Authorized>
            <fdev-btn
              label="骨架录入"
              @click="handleArchetypeDialogOpened('addMpassArchetype')"
              v-if="isManger(null)"
              normal
              ficon="add"
            />
          ...
          <fdev-td auto-width>
            <div v-if="isManger(props.row.manager)">
              <fdev-btn
                ...
                flat
                @click="
                  handleArchetypeDialogOpened('updateMpassArchetype', props.row)
                "
                v-if="isManger"
              />
            </div>
...
import { webArchetypeColums } from '@/modules/Component/utils/constants.js';
...
export default {
  ...
  computed: {
    ...mapState('userActionSaveComponent/archetypeManage/webArchetype', [
      'selectValue',
      'visibleColumns'
    ]),
    ...mapState('componentForm', ['mpassArchetypeList']),
    ...
    columns() {
      return webArchetypeColums(this.groups);
    }
  },
  watch: {
    pagination(val) {
  ...
  methods: {
    ...mapMutations('userActionSaveComponent/archetypeManage/webArchetype', [
      'updateSelectValue',
      'updatevisibleColumns'
    ]),
    ...mapActions('componentForm', ['queryMpassArchetypes']),
    ...mapActions('userForm', ['fetchGroup']),
    async init() {
      this.loading = true;
      await this.queryMpassArchetypes();
      this.loading = false;
    },
    ...
    async handleArchetypeDialogOpened(method, data) {
      ...
      if (method === 'updateMpassArchetype') {
        this.selectedData = {
          ...data,
          groupObj: data.group
