<template>
  <f-block>
    ...
    <!-- 迁移弹框 -->
    <f-dialog f-sl-sc v-model="addTransgerOpened" title="窗口选择">
...
import {
  ...
  webComponentIssuePageColums,
  webComponentIssuePageRecordColums
...
export default {
  ...
  data() {
    return {
      ...
      dialogOpen: false,
      updateData: {},
      columns: webComponentIssuePageColums,
      recordColumns: webComponentIssuePageRecordColums,
  ...
  computed: {
    ...
    issueMpassRecord: {
      get() {
        return this.$store.state.componentForm.mpassIssueRecord;
      },
      set(val) {
        this.$store.state.componentForm.mpassIssueRecord = val;
      }
    },
    // 当前窗口是否有正式发布版本
    isRelease() {
      if (this.mpassIssueRecord.length === 0) {
        return false;
      } else {
        let reg = /^[\d.]*$/;
        return this.mpassIssueRecord.some(val => {
          return reg.test(val.version);
        });
      }
    },
    columnsFilter() {
      let columnsItem = this.columns;
      if (this.isBusinessCom) {
        columnsItem.splice(
          ...
          0,
          {
            name: 'sit_date',
            label: '计划SIT日期',
            field: row => (row.sit_date ? row.sit_date : '-'),
            align: 'center'
          },
          {
            name: 'uat_date',
            label: '计划UAT日期',
            field: row => (row.uat_date ? row.uat_date : '-'),
            align: 'center'
          }
        );
      }
  ...
  methods: {
    ...
    linkTo(id) {
      ...
      this.$router.push({ name: 'WebHandlePage', params: { id } });
    },
    updateMpassDev(data) {
    ...
    isManager() {
      let comIds = this.mpassComDetail.manager.map(user => user.id);
    ...
    //组件管理员权限控制
    isComManage() {
      if (this.mpassComDetail && this.mpassComDetail.manager) {
        return this.mpassComDetail.manager.some(
          user => user.id === this.currentUser.id
        );
      }
    },
    ...
    async init() {
      ...
      this.isBusinessCom =
        this.$route.query.componentType === ComponentModelType.Business;
      await this.queryMpassDevIssue({ issue_id: this.$route.params.id });
      this.tabelData = this.mpassDevIssue;
      // 查询版本管理员等数据
      ...
      await this.queryMpassIssueRecord({
