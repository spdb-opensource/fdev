    ...
    <div>
      ...
      >
        <fdev-input
          ...
          :rules="[() => $v.createModel.interfacePath.required || '接口路径']"
        />
      </f-formitem>
      ...
      >
        <fdev-input
          ...
          type="textarea"
          :rules="[
            () => $v.createModel.functionDesc.required || '请选择接口功能说明'
          ]"
          v-model="createModel.functionDesc"
        />
      </f-formitem>
    </div>
...
import { updateApiRole } from '@/modules/Management/services/methods';
export default {
  name: 'updateApiRoleDlg',
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    getBugDetails() {
      this.createModel.interfacePath = this.getParams.interfacePath;
      this.createModel.roleIds = this.getParams.roles;
      this.createModel.functionDesc = this.getParams.functionDesc;
    }
  },
  async created() {
    this.getBugDetails();
