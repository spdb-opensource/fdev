<template>
  <div>
    <Loading :visible="loading['measureForm/queryMergeInfo']">
      <!-- 表格 -->
      <fdev-table
...
import { mergeRequestModel } from '@/modules/Measure/utils/constants';
import {
  getMergeRequestPagination,
  setMergeRequestPagination
} from '@/modules/Measure/utils/setting';
import { exportMergedInfo } from '@/modules/Measure/services/methods.js';
...
export default {
  ...
  watch: {
    ...
    'filterModel.end_time'(val) {
      this.filterData();
    },
    'filterModel.start_time'(val) {
      this.filterData();
    }
  },
  computed: {
    ...mapState('userActionSaveMeasure/mergeRequest', {
      filterModel: 'mergeRequestModel'
    }),
    ...mapState('global', ['loading']),
    ...mapState('measureForm', ['mergeInfo']),
    ...mapState('user', ['list']),
    ...mapState('userForm', {
      groups: 'groups'
    }),
  ...
  methods: {
    ...mapMutations('userActionSaveMeasure/mergeRequest', [
      'filterModelGroupId',
    ...
    ...mapActions('measureForm', ['queryMergeInfo']),
    ...
    async handleDownload() {
      let result = await resolveResponseError(() =>
        exportMergedInfo(this.filterModel)
      );
      exportExcel(result);
    },
    async init(props) {
