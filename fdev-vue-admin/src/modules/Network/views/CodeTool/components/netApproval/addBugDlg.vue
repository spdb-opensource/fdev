    ...
    <div>
      ...
      >
        <fdev-input
          ...
          type="textarea"
          :rules="[
            () => $v.createModel.problem.required || '请输入问题描述/评审建议'
          ]"
        />
      </f-formitem>
      <f-formitem
...
import { createProblemItemModel } from '@/modules/Network/utils/constants';
...
export default {
  ...
  methods: {
    ...
    ...mapActions('networkForm', ['queryProblemItem', 'addProblem']),
    verifyModel() {
      try {
    ...
    async saveEvaMgt() {
      let params = {
        ...
        itemType: this.createModel.itemType.key, //问题项
      ...
      try {
        this.loading = true;
        await this.addProblem(params);
        successNotify('新增问题项成功!');
