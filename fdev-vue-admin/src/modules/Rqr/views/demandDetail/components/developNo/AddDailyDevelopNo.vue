<template>
  <f-dialog
    @input="$emit('update:isShowAddDailyDevelopNo', false)"
    right
    title="新增研发单元"
    @before-show="initDialogData"
    :value="isShowAddDailyDevelopNo"
  >
      ...
      <div>
        ...
        <f-formitem label="研发单元牵头人" required>
          <fdev-select
            ...
            use-input
            multiple
            ref="reqImplUnitModel.implement_leader"
            v-model="reqImplUnitModel.implement_leader"
    ...
    <template v-slot:btnSlot>
      ...
      <fdev-btn
        label="取消"
        @click="$emit('update:isShowAddDailyDevelopNo', false)"
        outline
        dialog
      />
    </template>
...
import { queryPartInfo } from '@/modules/Rqr/services/methods.js';

export default {
  name: 'adddailydevelopNo', //新增日常研发单元
  props: {
    isShowAddDailyDevelopNo: {
      type: Boolean,
      default: false
    },
    demandId: {
  ...
  methods: {
    ...
    async AddDailyDevelopNo() {
      ...
      params.group = cpReqImplUnitModel.group.part_id;
      ...
      params.implement_leader_all = cpReqImplUnitModel.implement_leader;
      params.group = cpReqImplUnitModel.group.part_id;
      ...
      try {
        ...
        if (res && res.code && res.code != 'AAAAAAA') {
          ...
          // 成功
          addLigh = res.addLight;
        }
        // addLight为true发新增接口 ,其他值发补充接口.
        if (addLigh) {
          await this.addImplementUnit(params);
        } else {
          await this.supplyImplementUnit(params);
        }
      ...
      // 关闭弹框
      this.$emit('update:isShowAddDailyDevelopNo', false);
      // 刷新 研发单元列表列表
      // 小组 id  编号 刷新
      this.$emit(
        'updateList',
