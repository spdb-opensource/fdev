<template>
  <div class="typora-export">
    <div id="write" class="is-node">
      <h1>
        <a name="header-c1" class="md-header-anchor "></a>SOAP接口扫描规范
      </h1>
      <p>
        本规范适用于纳入开发协作平台Fdev的微服务应用，如该应用在接口调用层面属于SOAP接口使用方和提供方，则须遵守该接口规范，否则可能对SOAP接口扫描准确性造成影响。
      </p>
      ...
      <p>
        <span>请遵守规范 </span>
        <a
          href="http://xxx/ebank/doc/blob/master/微服务开发规范/微服务项目规范.md"
          target="_blank"
        >
          http://xxx/ebank/doc/blob/master/%E5%BE%AE%E6%9C%8D%E5%8A%A1%E5%BC%80%E5%8F%91%E8%A7%84%E8%8C%83/%E5%BE%AE%E6%9C%8D%E5%8A%A1%E9%A1%B9%E7%9B%AE%E8%A7%84%E8%8C%83.md
        </a>
      </p>
      ...
      <ol>
        <li>
          <p><span>存放soap接口相关文件路径：</span></p>
          <pre
            class="md-fences mock-cm"
            style="display:block;position:relative"
          >
兼容两种路径：
src\main\resources\config\packet\webservice
src\main\resources\packets\webservice
          </pre>
          <p>
            <span>注：</span>
            <strong
              ><span>项目主目录名及子目录名应与项目名称保持一致。</span></strong
            >
          </p>
          <p>
该目录下：该目录下,如该应用为soap接口调用方，则接口相关文件路径：<code>webservice\client\</code>
          </p>
          <p>
如该应用为soap接口提供方，则接口相关文件路径：<code>webservice\server\</code>
          </p>
        </li>
        <br />

        <li>
          <p>存放接口名称映射文档、接口头信息及路径配置文档路径:</p>
          <ul>
            <li>
              <p>如该应用为soap接口调用方，则接口名称映射文档为：<em><span>wsclient_mapping.xml</span></em></p>
              <p>路径为：<code>webservice\client\wsclient_mapping.xml</code></p>
              <p>或<code>webservice\client\client_mapping.xml</code>或<code>webservice\wsclient_mapping.xml</code></p>
            </li>

            <li>
              <p>如该应用为soap接口提供方，则接口名称映射文档为：<em>service_mapping</em>.xml</p>
              <p>
                <span> 路径为：</span>
                <code>webservice\server\service_mapping.xml</code>
                <span> 或 </span>
                <code>webservice\service_mapping.xml</code>
              </p>
            </li>
          </ul>

          <p>
            <span> 接口报文映射文档路径：</span
            ><code>\webservice\interface\</code><span> ,包含 </span>
            <em><span>EsbHeader.xml</span></em>
          </p>
          <p>
            <span> SoapHeader文件路径 : </span
            ><code>\webservice\wsdl\SoapHeader.xsd</code>
          </p>
        </li>
      </ol>
      ...
      <ol>
        <li>
          <p>接口mapping文档</p>
          <span>
扫描以该文件中的bean为准，逐条读取配置项，从key中获取交易码（服务id+操作id组成），value为接口映射别名
          </span>
          <span>
示例如下：<em>S010030004CrnAcctBalQry</em>为soap接口交易码，<em>QueryCurrentBalance为soap</em>接口别名
          </span>
          <pre
            class="md-fences mock-cm"
            style="display:block;position:relative"
          >
&lt;!-- 1504余额查询 --&gt;
&lt;prop key="{http://esb.spdbbiz.com/services/S010030004}ReqCrnAcctBalQry"&gt;
  QueryCurrentBalance
&lt;/prop&gt;</pre
          >
        </li>
        ...
        <li>
          <p><span>接口报文映射文件</span></p>
          <ul>
            <li>
              <p>文件命名：以接口别名命名，与<em>mapping</em>中的接口别名保持一一对应关系，如<em>QueryCurrentBalance.xml</em></p>
            </li>

            <li>
              <p>文件结构：包括三部分：xml声明、请求信息、响应信息</p>
              <p>对soap接口使用方：表示请求信息；表示响应信息</p>
              <p>对soap接口提供方：表示发送信息；表示接收信息</p>
            </li>

            <li>文件内容：应与esb提供的接口文档保持一致</li>
            <li>文件示例：为了展示效果，字段条数稍作删减</li>
            <li>备注：废弃、下线接口，请及时更新对应该映射文件</li>
          </ul>
        </li>
      </ol>
    </div>
...
.CodeMirror.cm-s-typora-default div.CodeMirror-cursor {
  ...
  border-left-style: solid;
  border-left-color: rgb(228, 98, 154);
}
...
p .md-image:only-child, p img:only-child {
  display: inline-block;
  width: 100%;
  text-align: center;
}
...
a.md-header-anchor.md-print-anchor {
...
.on-focus-mode blockquote {
  border-left-color: rgba(85, 85, 85, 0.117647);
}
