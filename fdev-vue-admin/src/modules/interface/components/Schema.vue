<template>
  <div class="typora-export">
    <div id="write" class="is-node">
      <h1>
        <a name="header-c1" class="md-header-anchor "></a
        >微服务JSONSchema使用规范
      </h1>
      <h3>1. 项目JSON Schema目录</h3>
      <p>微服务项目中JSON Schema需区分server和client，目录如下：</p>
      <pre class="md-fences mock-cm" style="display:block;position:relative">
服务端： src/main/resources/config/packet/restful/server/schema/
客户端： src/main/resources/config/packet/restful/client/schema/{ServiceId}/ </pre
      >
      <p>
        为避免跨公共微服务接口名称相同造成jsonschema定义文件冲突的情况,约定应用内按服务名目录存放文件.<br />
        客户端示例：src/main/resources/config/packet/restful/client/schema/ims-product-gold/
        [ps：路径修改后须同步修改schema-config配置]
      </p>
      <h3>2. JSONSchema文件规范</h3>
      <p>a. 文件命名</p>
      <p>
        Schema文件命名统一使用<code>交易码（transId）</code>作为文件名，json文件格式,如QueryBankInfo.json
      </p>
      <p>b. 文件结构</p>
      <p>
        Schema文件需包含三部分内容：Metadata（接口元数据）、Request（请求Schema）、Response（返回Schema），其中Request和Response节点只包含body部分Schema定义
      </p>
      <pre class="md-fences mock-cm" style="display:block;position:relative">
{
  &quot;Metadata&quot;: {
    &quot;ServiceId&quot;: &quot;服务id&quot;,
    &quot;InterfaceName&quot;: &quot;接口中文名称&quot;,
    &quot;Uri&quot;: &quot;http://serviceid/rest/transid&quot;,
  ...
  &quot;Response&quot;: {
      ...
      <p>c. 公共头部Schema文件RestHeader.json</p>
      <p>目录如下：</p>
      <pre class="md-fences mock-cm" style="display:block;position:relative">
 src/main/resources/config/packet/restful
      </pre>
      <p>
        为避免Schema文件冗余定义，抽取出请求头和返回头Schema定义，内容如下：
      </p>
      <pre class="md-fences mock-cm" style="display:block;position:relative">
{
  &quot;ReqHeader&quot;: {
  ...
  &quot;RspHeader&quot;: {
    ...
    &quot;properties&quot;: {
      ...
      <h3>3. JSON Schema示例</h3>
      <p>QueryBankInfo.json</p>
      <pre class="md-fences mock-cm" style="display:block;position:relative">
{
  &quot;Metadata&quot;: {
    &quot;ServiceId&quot;: &quot;ims-param-cardbin&quot;,
    &quot;InterfaceName&quot;: &quot;卡bin查询接口&quot;,
    &quot;Uri&quot;: &quot;http://ims-param-cardbin/rest/QueryBankInfo&quot;,
    &quot;RequestType&quot;: &quot;json/wsdl&quot;,
    &quot;RequestProtocol&quot;: &quot;http&quot;,
    &quot;InterfaceDescription&quot;: &quot;根据卡号查询对应卡bin信息&quot;
  },
  &quot;Request&quot;: {
