...
public class ErrorGenerator {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String dictName = "/"
					+ com.gotest.dict.ErrorConstants.class.getName().replace('.', '/');

			java.net.URL outUrl1 = com.gotest.generator.ErrorGenerator.class.getResource(dictName
					+ ".class");
			...
			prt.println(" */ ");
			prt.println("package com.gotest.dict;");
			...
			e.printStackTrace();
			logger.info("Exception in main() of ErrorGenerator: "
					+ e.toString() );
		}finally {
			if(prt != null){
				prt.close();
